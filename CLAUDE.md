...
- `frontend/` — Vite + TypeScript + Three.js(pnpm 管理)
  - `src/scene/` — Three.js/WebGL 描画層。**ユニットテスト免除**
...
- 例外は描画層(`frontend/src/scene/`)のみ。描画にロジックが混ざりそうになったら `logic/` へ抽出してテストする
...
- `/e2e` — Playwright による E2E 検証・スクリーンショット取得(フルスタック結合、Three.js 描画確認)
