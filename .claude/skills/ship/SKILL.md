...
1. **Which scope** was modified — use `git diff --name-only origin/main...HEAD` to identify changed files. If files span both `backend/` and `frontend/`, ask the developer which is primary.
2. **Whether an issue number was provided** in the arguments.
3. **Current branch name** — used to derive the PR title if needed.
4. **Whether a workflow state file exists** — check `plans/workflow_state.md`. If it exists, read it to understand what was in progress. This is informational context for generating the plan; delete it when filing the PR.
...
All gates (lint, typecheck, tests with coverage) must pass. If they don't, stop and tell the developer what failed. Fix formatting automatically with `make format`; if fixes require judgment, ask the developer.
...
Create a plan file:
- **If issue number provided:** `plans/issue-{N}-slug.md`
- **If no issue number:** `plans/session-{YYYY-MM-DD}-slug.md`
...
- **Assumptions about the other scope** (with docstring references if applicable)
...
```bash
git add plans/
git commit -m "docs: add retroactive plan for {slug}"
```

### 3. Version and Changelog

Check if the developer already bumped the version (`backend/pyproject.toml` or `frontend/package.json` depending on scope). If not:
...
### 4b. Doc-Parrot (backend changes only)
...
The parrot is not a gate. It produces an artifact you act on with judgment. (Frontend-only changes: skip and note "doc-parrot: n/a" in the findings file.)
...
- **If issue number provided:** `plans/issue-{N}-slug-findings.md`
- **If no issue number:** `plans/session-{YYYY-MM-DD}-slug-findings.md`
...
If `plans/workflow_state.md` exists, delete it before pushing:
```bash
git rm --ignore-unmatch plans/workflow_state.md
...
```bash
PR_URL=$(gh pr create --title "{concise title}" --body "{body}" --base main)
...
Plan: plans/session-{date}-slug.md
```
...
```bash
gh issue comment {N} --body "Addressed in PR #${PR_NUMBER}.
