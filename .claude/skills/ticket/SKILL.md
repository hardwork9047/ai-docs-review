...
- Module and function **docstrings** (grep for docstrings — do NOT read full source files)
- **README.md**, **CLAUDE.md**, and **CHANGELOG.md**
- Past **plan files** in `plans/`
...
2. **Ask clarifying questions.** Draw out what's missing:
   ...
   - Which scope does this belong to — `backend` or `frontend`?
   ...
   - Are there related issues? (Check with `gh issue list --limit 30 --json number,title`)
...
- **Acceptance scenarios are plain language**, not test code. "When X happens, Y should be true" — not `assert foo.bar() == expected`. The implementing agent translates them into failing tests first (TDD), so make each scenario concretely checkable.
...
```bash
gh issue create --title "{concise title}" --body "{the issue body}"
```

(If the repo has an `AI-Generated` label, add `--label "AI-Generated"`; if not, skip the label rather than failing.)

Tell the human the issue number so they can hand it to an implementing agent via `/implement {scope} {issue#}`.
